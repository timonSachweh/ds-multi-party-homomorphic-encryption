...
run_services() {
    ...
    source '${main_path}/env/server.env'; go run ${main_path}aggregation-server/cmd/main.go"

    screen -dmS "he-client-1-go" bash -c "
    source '${main_path}/env/client.env'; go run ${main_path}he-client/cmd/main.go"

    screen -dmS "he-client-1-python" bash -c "
    source '${main_path}/env/client.env'; pyenv activate ml; python ${main_path}python-client/src/main.py"
